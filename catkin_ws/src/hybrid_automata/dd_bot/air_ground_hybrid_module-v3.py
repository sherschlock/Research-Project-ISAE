...
# Adding Safety Protocol for takeoff and landing (different from the safe landing algorithm which is triggered in an emergency if the battery goes below 40%)
...
try:
	while not rospy.is_shutdown():
		...
		if (abs(currPosition.x - desiredPosition.x) < 0.1 and abs(currPosition.y - desiredPosition.y) < 0.1 and abs(currPosition.z - desiredPosition.z) < 0.1):
			...
			takeoff_wp.pose.position.x = currPosition.x
			takeoff_wp.pose.position.y = currPosition.y
			takeoff_wp.pose.position.z = 1
			pub_wp.publish(takeoff_wp)
			while not rospy.is_shutdown():
				try:
					...
					if phase == 'taking off':
						...
						if desiredPosition.z > 0.4:							# aerial coordinates
							pub_wp.publish(msg)
							if (abs(currPosition.x - desiredPosition.x) < 0.1 and abs(currPosition.y - desiredPosition.y) < 0.1 and abs(currPosition.z - desiredPosition.z) < 0.1):
								print('Target reached!\n')
								desiredPosition,missionTime = user_input()
								...
								if dist > flight_radius:									# This checks for the condition against the new waypoint received
									offb_node.stop()
									break
						elif desiredPosition.z < 0.4:									# ground coordinates
							landingfrom_wp = PoseStamped()
							landingfrom_wp.pose.position.x = desiredPosition.x
							landingfrom_wp.pose.position.y = desiredPosition.y
							landingfrom_wp.pose.position.z = 1
							pub_wp.publish(landingfrom_wp)
							if (abs(currPosition.x - desiredPosition.x) < 0.1 and abs(currPosition.y - desiredPosition.y) < 0.1):
								print('Target reached!\n')
								offb_node.stop()
								desiredPosition,missionTime = user_input()
								msg.pose.position.x = desiredPosition.x
