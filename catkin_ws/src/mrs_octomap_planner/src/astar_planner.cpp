...
// The following few lines are definitions for different member functions that can be found as structures in the .hpp file included
// above. Struct and Class keywords do very similar things. The only difference is that variables in struct are public by default
...
// The function uses the C++ standard library hash function to hash the three components of the Node object's octomap::OcTreeKey
// member variable key. The three components of the key variable correspond to the x, y, and z coordinates of the Node in the search
// graph
...
// Compares the occupancy probability of 2 leaf nodes
...
*/
// Adds cubes on the start and goal points
/*
...
// We start from the farthest reachable point that exists in the tunnel
/*
...
*/
  auto start = tree.coordToKey(map_start);
  auto goal           = tree.coordToKey(map_goal);
  auto map_query = mapping_tree->search(goal_coord);
  if (map_query == NULL) {
    ROS_INFO("Goal Coordinates not found in the map");
  }
// Takes into account special case where the start and goal points are already close and the search is unnecessary
/*
  if (distEuclidean(map_start, map_goal) <= 2 * planning_tree_resolution) {
...
*/
  ...
  ROS_INFO_STREAM("[Astar]: Start : " << map_start.x() << ", " << map_start.y() << ", " << map_start.z());
  ROS_INFO_STREAM("[Astar]: Goal: " << map_goal.x() << ", " << map_goal.y() << ", " << map_goal.z());

// Defines the first node. The distEuclidean function needs the tree as well because the start and goal coordinates need to be
// converted into keys before their distance is computed.
...
// Keeps track of evaluated nodes and nodes to be evaluated
...
// In case time runs out, the planner takes the best node as the goal and delivers a sub optimal path
...
// If a node is found close enough to the goal, the path is delivered to that node
...
// Main A* search algorithm
    ...
    // expand
    // Gets a list of neighbors of the current node
    ...
    for (auto &nkey : neighbors) { // Iterating through the neighbor list
...
// Checking to see whether the node already exists in the open or closed sets
...
/* The function takes an original OcTree object tree and creates a new OcTree object resampled_tree with a specified resolution.
The new resampled_tree has the same occupancy thresholds, hit and miss probabilities, and clamping thresholds as the original tree.
The resampled tree is used to create a planning tree for the A* planner. */
...
// Ensuring that the root node of the resampled tree is not occupied

  octomap::OcTreeKey key = resampled_tree->coordToKey(0, 0, 0, resampled_tree->getTreeDepth());
  resampled_tree->setNodeValue(key, 0.0);

/* The purpose of setting p_min and p_max is to define the boundaries of the region of interest around the starting point.
This region will be used to extract a submap from the resampled octree that is centered at the starting point and has a size of
2*submap_distance in each dimension. The submap is used to limit the search space of the planner to a smaller region of the map,
which can significantly reduce the computational cost of the planning algorithm. By choosing the boundaries of the submap to be
submap_distance units away from the starting point in each direction, we ensure that the submap is large enough to capture enough
information about the environment around the starting point, while also being small enough to avoid unnecessarily large search spaces.*/
...
/* This code iterates through all the leaf nodes of the original tree object that fall within the bounding box specified by p_min and p_max.
For each leaf node, it retrieves its key, depth, and occupancy status using the iterator it. Then, for each leaf node, a corresponding leaf node is
created in the resampled_tree object by using the coordToKey function to calculate the key for the new node. The touchNode function is called to
create the new node if it doesn't exist already.Finally, the occupancy probability of the old node is checked. If it is occupied, the corresponding
new node is assigned a log-odds value of 1.0, indicating that it is highly likely to be occupied. If the old node is not occupied, the corresponding
new node is assigned a log-odds value of -1.0, indicating that it is highly likely to be free. The counters counter_occ, counter_free, and counter
are used to keep track of the number of occupied nodes, free nodes, and total nodes processed, respectively. */
  ...
  /* This is part of the code the obstacles are inflated. Nodes that are within a certain distance are deemed occupied while farther away nodes are free.*/
  auto edf = euclideanDistanceTransform(resampled_tree, orig_coord, radius); // This calculates the distance of every voxel from the nearest obstacle.
